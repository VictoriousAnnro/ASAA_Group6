...
\begin{enumerate}
    ...
    \item What trade-offs arise from the choosen technologies?** DELETE?
\end{enumerate}
...
%TODO AM
% QAs from prev section used to formulate requirements here. 
% then present tactics
...
%**not sure if only to include RQs for our QAeses??**
\begin{center}
\begin{tabular}{ | m{0.5cm} | m{4cm}| m{1cm} | m{1cm} | }
 \hline
 Req ID & Description & Type & QA \\ 
 \hline
 id & Customer can choose from a list of available cars & Func & Us\\ 
 \hline
 id & Customer should be able to chose a car and customize the car's model, engine and color & type & qa\\ 
 \hline
 id & Scheduler should prepare a placed order for production by creating and saving a production recipe & type & Perf\\
 \hline
 id & Scheduler should keep track of an order's status (ready, in progress, completed) & type & per\\
 \hline
 id & The website should respond to user actions in less than 3 seconds & type & Perf\\
 \hline
 id & Any order placed on the website should be saved and marked as 'ready' for production in database within 10 time units & type & Perf\\ 
 \hline
\end{tabular}
\label{ReqTabel}
\end{center}
...
\subsection{Features}
Figure \ref{FeatureDiagram} shows the features for the car production system that will realize the defined requirements. The system will have a website, from which the customer may browse and choose a car to customize. The Scheduler will process the orders by creating a production recipe for each order. The Scheduler also manages the status of each order, setting status to 'ready' for prodduction when the corresponding production recipe has been created and stored in the system. 
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{featureDiagram.jpg}
    \caption{Feature Diagram - CHANGE ORDERSTATUS}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{SchedulerFAA.jpg}
    \caption{Functional Analysis Model of Scheduler}
    \label{schedulerFAA}
\end{figure}

\subsection{Finite-State Machine/Automata}
Behavioral model, finite-state machine (FSM)...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{WebsiteSM.pdf}
    \caption{sm website - FIX SENDING MESSAGE TO SCHEDULER}
    \label{websiteSM}
\end{figure}
...
The time will be measured in (mili)seconds. 
