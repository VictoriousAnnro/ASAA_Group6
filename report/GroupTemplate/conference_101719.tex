...
\author{
    ...
    \IEEEauthorblockA{
        ...
        Email: \IEEEauthorrefmark{1} \textnormal{\{anrom,,student3,student4,student5\}}@mmmi.sdu.dk
    }
}
...
% ... Shoul review the state of the art consisting of 8-10 papers and should contextualize how this study provides new knowledge to the field. 
...
This Section introduces the use cases, of which there are XX.
%\subsection{Add Customer Order to production queue}
%Actors: Production Scheduler, Website\\
%Preconditions: The order has been placed by the customer through the website.\\
%Steps:
%\begin{itemize}
...
%\end{itemize}
%Postconditions: An order is stored and marked as 'ready' for production. 
...
%The customer accesses the company website to choose their preferred car. The web server queries the database for the chosen model and fetches available customization options. After the customer chooses preferred customizations, they place the order. The web server saves the order in the database.
%Postconditions: A new customer order is saved in the database

%\subsection{Schedule Production Run}
%Actors: Production Scheduler, Production Orchestrator
%Preconditions: The production order has been made by the customer, and is stored in the database. 
%Steps:
%The Production Scheduler retrieves the order from the database and breaks it down into a JSON production recipe. Then fetches available time slots and resources. Prosuction Schedular selects an available time slot, schedules the production of the order, and notifies the Production Orchestrator.
%Postconditions: A production run is scheduled, and all affected components are notified.
\subsection{Execute production run}
Actors: Production Orchestrator, Production Scheduler, Production Floor Components
Preconditions: The recipe for a customer order has been created and stored, and the order has been marked as ready for production. Production Floor components are all available to work.
Steps: 
Steps:
\begin{itemize}
    \item Orchestrator contacts the Scheduler for next ready order
    \item Scheduler queries the oldest order that is marked as ready in database
    \item Scheduler fetches the relevant recipe from database, and sends it to the Orchestrator
    \item Scheduler updates the order's status from 'ready' to 'inProgress'
    \item Orchestrator delegates the steps in the recipe to production floor components and starts production
\end{itemize}
Postconditions: Order status is 'inProgress' and production run has started.

%The production run for a placed order has been scheduled. 
%Steps:
%Production Orchestrator fetches the production recipe and coordinates the production run by assigning tasks to the available components on the production floor. Once all tasks are assigned, Production Orchestrator instructs the components to begin execution and monitors the components.
%Postconditions: Components continue running until the production run is complete.
\subsection{Check status of production floor components}
Actors: Production Manager, Production Orchestrator
Preconditions: Production is running, a component on the production floor is not acting as expected
Steps:
Production Manager signs into the production software and selects the "Check status". Production Orchestrator fetches logs and real-time status information from each component on the production floor. After reviewing the logs and the current status of the faulty component, the Production manager pauses the component to allow for maintenance. When the component is fixed, the production manager resumes the component operation.
Postconditions: Affected component is acting as expected.

\subsection{Abort production run}
Actors: Production Manager, Production Scheduler
Preconditions: Production is running
Steps:
Production Manager logs into the production software and navigates the ”Manage Production Run” option.Production Scheduler retrievsand displays current production overview. When the Production Manager selects the ”Stop Production Run” option the Production Scheduler confirms the abort, and notifies related components. Each active components abort their current tasks, and return to default operational state.
Postconditions: Production run is aborted, and all affected components return to default state.

\subsection{Schedule hardware maintenance}
Actors: Production Manager, Production Scheduler
Preconditions: Production is running
Steps:
Production Manager logs into the production software and selects the ”Manage Production Run” option. Production Scheduler fetches an overview of production. Production Manager selects the ”Schedule Maintenance” option. Production Scheduler confirms the date and notifies related components.
Postconditions: Components continue running, until the scheduled maintenance date.

\subsection{Schedule software update}
Actors: Production Manager, Production Scheduler
Preconditions: Production is running
Steps:
Production Manager signs into the production software and selects the ”Manage Production Run” option. The Production Scheduler retrieves the current overview of production. Production Manager then selects the ”Schedule software update”, after which the Production Scheduler confirms the date and notifies all related components.
Postconditions: Components continue running, until the scheduled maintenance date.
